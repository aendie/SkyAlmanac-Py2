...
def rise_set_error(y, lats, t0):
    if config.logfileopen:
        ...
        if len(y) > 2:
            config.writeLOG("%s" %y[2])
        if len(y) > 3:
            config.writeLOG("%s" %y[3])
        config.writeLOG("\n%s" %t0.utc_iso())
    else:
...
def sunGHA(d):              # used in sunmoontab(m)
    ...
    ghas = ['' for x in xrange(24)]
    decs = ['' for x in xrange(24)]
    degs = ['' for x in xrange(24)]
...
def sunSD(d):               # used in sunmoontab(m)
    ...
    sdsm = "%0.1f" %(sds * 60)	# convert to minutes of arc
...
def moonGHA(d):             # used in sunmoontab(m)
    ...
    GHAupper = [-1.0 for x in xrange(24)]
    GHAlower = [-1.0 for x in xrange(24)]
    gham = ['' for x in xrange(24)]
    decm = ['' for x in xrange(24)]
    degm = ['' for x in xrange(24)]
    HPm  = ['' for x in xrange(24)]
    for i in range(len(dec.degrees)):
...
##        if raIDL > 24: raIDL = raIDL - 24
        ...
        HPm[i] = "%0.1f'" %(HP * 60)	# convert to minutes of arc
...
def moonVD(d0,d):           # used in sunmoontab(m)
    ...
    moonVm = ['' for x in xrange(24)]
    moonDm = ['' for x in xrange(24)]
    for i in range(len(dec.degrees)):
        ...
        moonVm[i] = "%0.1f'" %(Vdm)
        D1 = dec.degrees[i]
        moonDm[i] = "%0.1f'" %((D1-D0) * 60)	# convert to minutes of arc
...
def vdm_Venus(d):           # used in planetstab(m)
    ...
    RAcorrm = "%0.1f" %(sha * 60)	# convert to minutes of arc
    Dcorr = dec1.degrees - dec0.degrees
    Dcorrm = "%0.1f" %(Dcorr * 60)	# convert to minutes of arc
...
# Venus
    ...
    hpvenus = "%0.1f" %((math.tan(6371/(vau.au*149597870.7)))*60*180/math.pi)
...
# Mars
    ...
    hpmars = "%0.1f" %((math.tan(6371/(mau.au*149597870.7)))*60*180/math.pi)
